...
7.  **Existence (∃):** Sequence element x_t.
8.  **Distinction (¬):** Current vs. Previous state.
9.  **Disjunction (∨):** Time step separation.
10. **Conjunction (∧):** Dual integration (Input + Hidden).
11. **Transition (→):** Hidden state update h_t.
12. **Modular Closure (↔):** Sequence output.
13. **Recursion (∀):** **Hidden state fed back as input (Local Loop).**
...
| **Distinction (8)** | 21. Active/Inactive | 28. Firing/Silence | 35. Threshold (θ) | 42. Output vs Target | 49. Error detected | 56. Error magnitude |
...
| **Recursion (13)** | 26. **h_t → h_{t-1} loop** | 33. **Next time step** | 40. **State preservation** | 47. Network architecture | 54. Next Epoch begins | 61. Optimizer step |
...
### The 13 Primary Categories (Generators)
*(1-6 remain identical to ANN)*
7.  **Existence (∃):** Node feature x_v.
8.  **Distinction (¬):** Node vs Edge.
9.  **Disjunction (∨):** **Adjacency Matrix (Explicit Topology).**
10. **Conjunction (∧):** **Message Passing along edges.**
11. **Transition (→):** Node embedding update.
12. **Modular Closure (↔):** Graph-level readout (Pooling).
13. **Recursion (∀):** Layer-wise message propagation.
...
| **Distinction (8)** | 21. Connected/Unconnected | 28. Firing/Silence | 35. Threshold (θ) | 42. Output vs Target | 49. Error detected | 56. Error magnitude |
...
| **Recursion (13)** | 26. h_v^{(k)} → h_v^{(k+1)} | 33. Next layer triggers | 40. Iterative firing | 47. Network architecture | 54. Next Epoch begins | 61. Optimizer step |
...
### The 13 Primary Categories (Generators)
*(1-6 remain identical to ANN)*
7.  **Existence (∃):** Token embedding + Positional encoding.
8.  **Distinction (¬):** Query, Key, Value projections.
9.  **Disjunction (∨):** Masking (causal or padding).
10. **Conjunction (∧):** **Dynamic Attention Scores (QK^T).**
11. **Transition (→):** Multi-head subspace projection.
12. **Modular Closure (↔):** LayerNorm + Residual connection.
13. **Recursion (∀):** Stacked encoder/decoder blocks.
...
| **Distinction (8)** | 21. **Q, K, V vectors** | 28. Firing/Silence | 35. Threshold (θ) | 42. Output vs Target | 49. Error detected | 56. Error magnitude |
